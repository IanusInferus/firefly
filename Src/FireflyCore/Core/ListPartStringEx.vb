...
''' </summary>
Public Class ListPartStringEx(Of T)
    Implements ICollection(Of T), IEquatable(Of ListPartStringEx(Of T)), IComparable(Of ListPartStringEx(Of T)), ICloneable

    Private Internal As IList(Of T)
    Private HashCode As Int32

    Private InternalList As IList(Of T)
    ...
    Public Sub New(ByVal Data As IList(Of T), ByVal Offset As Integer, ByVal Length As Integer)
        If Data Is Nothing Then Throw New ArgumentNullException
        Internal = New ListPart(Of T)(Data, Offset, Length)
    ...
    ''' <summary>已重载。考虑到效率原因，不会复制数据，而是直接引用数据，因此传入的数组不得改变</summary>
    Public Sub New(ByVal Data As IList(Of T), ByVal Offset As Integer, ByVal Length As Integer, ByVal HashCode As Int32)
        If Data Is Nothing Then Throw New ArgumentNullException
        Internal = New ListPart(Of T)(Data, Offset, Length)
    ...
    Public Sub New(ByVal BaseString As ListPartStringEx(Of T), ByVal ExpandLength As Integer)
        Internal = New ListPart(Of T)(BaseString.InternalList, BaseString.Offset, BaseString.Length + ExpandLength)

        InternalList = BaseString.InternalList
        Offset = BaseString.Offset
        Length = BaseString.Length + ExpandLength

        Dim hash As Integer = BaseString.HashCode
        For Each v In New ListPart(Of T)(BaseString.InternalList, BaseString.Offset + BaseString.Length, ExpandLength)
            hash = ((hash << 5) Xor v.GetHashCode Xor ((hash >> 2) And &H3FFFFFFF))
        Next
        HashCode = hash
    End Sub
    ...
    ''' <summary>已重载。延长已有的串。考虑到效率原因，不会复制数据，而是直接引用数据，因此传入的数组不得改变</summary>
    Public Sub New(ByVal BaseString As ListPartStringEx(Of T), ByVal ExpandLength As Integer, ByVal HashCode As Int32)
        Internal = New ListPart(Of T)(BaseString.InternalList, BaseString.Offset, BaseString.Length + ExpandLength)
    ...
    Public Overloads Function Equals(ByVal other As ListPartStringEx(Of T)) As Boolean Implements System.IEquatable(Of ListPartStringEx(Of T)).Equals
    ...
    Public Overrides Function Equals(ByVal obj As Object) As Boolean
        ...
        Dim s = TryCast(obj, ListPartStringEx(Of T))
    ...
    Public Function CompareTo(ByVal other As ListPartStringEx(Of T)) As Integer Implements System.IComparable(Of ListPartStringEx(Of T)).CompareTo
    ...
    Public Function Reverse() As ListPartStringEx(Of T)
        Return Reverse(0, Internal.Count)
    End Function

    Public Function Reverse(ByVal Index As Integer, ByVal Count As Integer) As ListPartStringEx(Of T)
        ...
        Next
        Return New ListPartStringEx(Of T)(Result)
    End Function

    Public Shared Widening Operator CType(ByVal l As ListPartStringEx(Of T)) As StringEx(Of T)
    ...
    Public Function IndexOf(ByVal Value As ListPartStringEx(Of T)) As Integer
        Return IndexOf(Value, 0, Internal.Count)
    End Function

    Public Function IndexOf(ByVal Value As ListPartStringEx(Of T), ByVal Index As Integer) As Integer
        Return IndexOf(Value, Index, Internal.Count - Index)
    End Function

    Public Function IndexOf(ByVal Value As ListPartStringEx(Of T), ByVal Index As Integer, ByVal Count As Integer) As Integer
        If Index < 0 OrElse Index >= Internal.Count Then Throw New ArgumentOutOfRangeException
        If Index + Count > Internal.Count Then Throw New ArgumentOutOfRangeException
        Dim ec = EqualityComparer(Of T).Default
    ...
    Public Function LastIndexOf(ByVal Value As ListPartStringEx(Of T)) As Integer
        Return LastIndexOf(Value, Internal.Count - 1, Internal.Count)
    End Function

    Public Function LastIndexOf(ByVal Value As ListPartStringEx(Of T), ByVal Index As Integer) As Integer
        Return LastIndexOf(Value, Index, Index + 1)
    End Function

    Public Function LastIndexOf(ByVal Value As ListPartStringEx(Of T), ByVal Index As Integer, ByVal Count As Integer) As Integer
        If Index < 0 OrElse Index >= Internal.Count Then Throw New ArgumentOutOfRangeException
        If Index >= Internal.Count Then Throw New ArgumentOutOfRangeException
        Dim ec = EqualityComparer(Of T).Default
    ...
    Public Shared Operator =(ByVal Left As ListPartStringEx(Of T), ByVal Right As ListPartStringEx(Of T)) As Boolean
    ...
    Public Function Substring(ByVal Index As Integer, ByVal Length As Integer) As ListPartStringEx(Of T)
        Return New ListPartStringEx(Of T)(New ListPart(Of T)(Internal, Index, Length))
    End Function

    Public Function Substring(ByVal Index As Integer) As ListPartStringEx(Of T)
        Return New ListPartStringEx(Of T)(New ListPart(Of T)(Internal, Index, Internal.Count - Index))
    End Function
End Class

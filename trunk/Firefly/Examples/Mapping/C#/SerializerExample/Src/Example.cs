...
public sealed class Example
{
    ...
    {
        //创建自定义二进制序列化器实例
        var mbs = new MyBinarySerializer();
        //创建自定义XML序列化器实例
        var mxs = new MyXmlSerializer();

        //创建数据
        var Obj = new DataObject();
        Obj.DataEntries.Add("DataEntry1", new DataEntry { Name = "DataEntry1", Data = new Byte[] { 1, 2, 3, 4, 5 }, Attribute = "Version2Only" });
        Obj.DataEntries.Add("DataEntry2", new DataEntry { Name = "DataEntry2", Data = new Byte[] { 6, 7, 8, 9, 10 }, Attribute = "Version2Only" });
        Obj.ImmutableDataEntries.Add("ImmutableDataEntry1", new ImmutableDataEntry<Byte[]>("ImmutableDataEntry1", new Byte[] { 1, 2, 3, 4, 5 }));
        Obj.ImmutableDataEntries.Add("ImmutableDataEntry2", new ImmutableDataEntry<Byte[]>("ImmutableDataEntry2", new Byte[] { 6, 7, 8, 9, 10 }));

        //二进制序列化
        var BinVersion1Bytes = mbs.WriteVersion1(Obj);
        var BinVersion2Bytes = mbs.Write(Obj);

        //二进制反序列化
        var BinVersion1RoundTripped = mbs.Read(BinVersion1Bytes);
        var BinVersion2RoundTripped = mbs.Read(BinVersion2Bytes);

        //XML序列化
        var XmlVersion1XElement = mxs.WriteVersion1(Obj);
        var XmlVersion2XElement = mxs.Write(Obj);

        //XML反序列化
        var XmlVersion1RoundTripped = mxs.Read(XmlVersion1XElement);
        var XmlVersion2RoundTripped = mxs.Read(XmlVersion2XElement);

        var Setting = new XmlWriterSettings { Encoding = Console.Out.Encoding, Indent = true, OmitXmlDeclaration = false };
        using (var w = XmlWriter.Create(Console.Out, Setting))
        {
